# Agent Skills

This repository is a home for many named Codex skills.

Each skill lives in its own top-level directory and should include a `SKILL.md` file plus any optional `agents/`, `scripts/`, `references/`, or `assets/` needed to make the skill reliable and reusable.

## Skills

The list below is kept in alphabetical order by skill name.

| Skill | Description |
| --- | --- |
| [`global-lab-notebook`](./global-lab-notebook/SKILL.md) | Create and maintain a concurrency-safe global lab notebook outside project roots, with unique experiment directories, append-only indexing, and per-experiment logs for work that spans multiple repos or tasks. |
